...
export const tooltip = (el: HTMLElement) => {
    ...
    const addTooltip = () => {
        ...
        tooltipDiv.forEach((i: HTMLElement) => {
            let top = parentCoords.top + window.scrollY - i.offsetHeight - 10;
            let bottom = parentCoords.bottom + window.scrollY + 10;
            let left = parentCoords.left + window.scrollX;
            let right = parentCoords.right + window.scrollX - i.offsetWidth;
            let center = parentCoords.left + window.scrollX + (el.offsetWidth - i.offsetWidth) / 2;
